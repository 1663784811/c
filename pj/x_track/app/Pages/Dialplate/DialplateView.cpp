...
void DialplateView::TopInfo_Create(lv_obj_t* par)
{
    ...
    lv_obj_set_size(cont, LV_HOR_RES, 284);

    lv_obj_set_style_bg_opa(cont, LV_OPA_COVER, 0);
    lv_obj_set_style_bg_color(cont, lv_color_hex(0x333333), 0);

    lv_obj_set_style_radius(cont, 54, 0);
    lv_obj_set_y(cont, -72);
    ui.topInfo.cont = cont;

    lv_obj_t* label = lv_label_create(cont);
    lv_obj_set_style_text_font(label, Resource.GetFont("bahnschrift_96"), 0);
    ...
    lv_obj_align_to(label, ui.topInfo.labelSpeed, LV_ALIGN_OUT_BOTTOM_MID, 0, 28);
...
void DialplateView::BottomInfo_Create(lv_obj_t* par)
{
    ...
    lv_obj_align(cont, LV_ALIGN_TOP_MID, 0, 280);
...
void DialplateView::SubInfoGrp_Create(lv_obj_t* par, SubInfo_t* info, const char* unitText)
{
    ...
    lv_obj_t* label = lv_label_create(cont);
    lv_obj_set_style_text_font(label, Resource.GetFont("bahnschrift_24"), 0);
    lv_obj_set_style_text_color(label, lv_color_white(), 0);
...
void DialplateView::BtnCont_Create(lv_obj_t* par)
{
    ...
    lv_obj_align_to(cont, ui.bottomInfo.cont, LV_ALIGN_OUT_BOTTOM_MID, 0, 60);
    ...
    ui.btnCont.btnMap = Btn_Create(cont, Resource.GetImage("locate"), -140);
    ui.btnCont.btnRec = Btn_Create(cont, Resource.GetImage("start"), 0);
    ui.btnCont.btnMenu = Btn_Create(cont, Resource.GetImage("menu"), 140);
}
...
lv_obj_t* DialplateView::Btn_Create(lv_obj_t* par, const void* img_src, lv_coord_t x_ofs)
{
    ...
    lv_obj_set_style_width(obj, 55, LV_STATE_PRESSED);
    lv_obj_set_style_height(obj, 35, LV_STATE_PRESSED);
