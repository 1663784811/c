...
void LiveMapView::Style_Create()
{
    ...
    lv_style_set_text_font(&ui.styleLabel, Resource.GetFont("bahnschrift_24"));
    lv_style_set_text_color(&ui.styleLabel, lv_color_white());

    lv_style_init(&ui.styleLine);
    lv_style_set_line_color(&ui.styleLine, lv_color_hex(0x3db2ff));
    lv_style_set_line_width(&ui.styleLine, 10);
...
void LiveMapView::ZoomCtrl_Create(lv_obj_t* par)
{
    ...
    lv_obj_set_pos(cont, lv_obj_get_style_width(par, 0) - 80, 80);
    ui.zoom.cont = cont;
...
void LiveMapView::SportInfo_Create(lv_obj_t* par)
{
    ...
    lv_obj_align(obj, LV_ALIGN_BOTTOM_LEFT, -20, 20);
    ...
    lv_obj_align(label, LV_ALIGN_LEFT_MID, 40, -20);
    ...
    lv_obj_set_style_text_font(label, Resource.GetFont("bahnschrift_24"), 0);
    lv_obj_set_style_text_color(label, lv_color_white(), 0);
    lv_obj_align_to(label, ui.sportInfo.labelSpeed, LV_ALIGN_OUT_BOTTOM_MID, 0, 6);

    ui.sportInfo.labelTrip = ImgLabel_Create(obj, Resource.GetImage("trip"), 10, 30);
    ui.sportInfo.labelTime = ImgLabel_Create(obj, Resource.GetImage("alarm"), 10, 60);
}
...
lv_obj_t* LiveMapView::ImgLabel_Create(lv_obj_t* par, const void* img_src, lv_coord_t x_ofs, lv_coord_t y_ofs)
{
    ...
    lv_obj_align(img, LV_ALIGN_TOP_MID, 10, y_ofs);
